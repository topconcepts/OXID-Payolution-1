...
[{if ($oViewConf->getTopActiveClassName() === 'alist' && $config->showInstallmentPriceOnCategoryPage() && $validation->installmentAvailable($price)) }]
    ...
    <span class="payoOldPrice oldPrice" data-price="[{$price->getPriceInActCurrency($price->getPrice())}]" data-currency="[{$currency->sign}]"  style="display: none;">
        <span class="payoPrice"></span> / [{oxmultilang ident="PAYOLUTION_PAYMENT_PER_MONTH"}]
